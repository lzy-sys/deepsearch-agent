...
- **最终答案流式输出**
  - 主智能体回答按 token 增量经 WebSocket 推送到前端，形成打字机式的实时输出，过程与结果所见即所得。
- **会话级上下文隔离**
...
| 知识库助手     | 发现可用知识库，并基于内部文档回答问题 | `list_knowledge_bases`、`ask_knowledge_base`                   |
...
| 容器化部署     | `Docker` / `docker-compose` / `Nginx`             | 后端多阶段镜像，前端静态托管与 `/api`、`/ws` 反向代理                           |
...
├── docs/
│   ├── ARCHITECTURE.md             # 系统架构与事件链路设计文档
│   ├── RESUME.md                   # 简历项目描述模板与面试 Q&A
│   └── FRONTEND_GUIDE.md           # 前端速成理解指南
├── Dockerfile                      # 后端多阶段构建镜像
├── docker-compose.yaml             # 全栈一键启动（MySQL + 后端 + 前端）
├── .env.example                    # 环境变量示例
...
- 可用的大模型 API Key
- Tavily API Key
- 内部知识库文档（可选，`docs/knowledge_base/` 已内置示例）
...
### 10. Docker 一键启动（全栈，可选）

仓库提供全栈 Docker Compose：`MySQL + 后端 + 前端(Nginx)`，一条命令拉起整套系统。
...
- MySQL：本机 `3307` 端口（可用 `MYSQL_PORT` 覆盖）
...
`evals/` 提供 12 个典型研搜任务（网络搜索、数据库查询、上传文件、混合场景），每个任务附带参考答案要点。运行器会真实执行一次完整研搜，再用大模型按「完整性 / 准确性 / 结构化」三个维度打分，产出 CSV 与 Markdown 评测报告。
