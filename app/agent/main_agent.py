...
# 2. subagents 放网络、数据库、知识库三类信息获取助手
...
async def run_deep_agent(task_query, session_id):
    ...
    try:
        ...
        #  - updates ：产出各节点最新状态（工具调用、子智能体调用、最终结果），沿用原有解析逻辑
        #  - messages：产出主智能体模型回答的 token 级增量，实时推送给前端形成打字机效果
        async for mode, chunk in main_agent.astream(
            ...
            if mode == "messages":
                # messages 模式下 chunk 为 (message_chunk, metadata) 元组
                ...
                if (
                    ...
                    # 只推送纯文本增量，跳过工具调用参数等非回答内容
                    if isinstance(delta, str) and delta:
                        monitor.report_answer_delta(delta)
                continue

            # updates 模式下 chunk 形如 {"model": {"messages": [...]}}，这里主要关心模型最新消息
    ...
    # 返回最终答案文本：Web 服务忽略返回值，评测集据此对研搜质量打分
    return final_answer
