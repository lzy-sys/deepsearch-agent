...
class ToolMonitor:
    ...
    def _emit(
        ...
        payload = {
            ...
            "message": message,
            "data": data or {},
            # 带时区的时间戳（.astimezone()），避免容器 UTC 与浏览器本地时区解析偏差
    ...
    def report_answer_delta(self, delta: str) -> None:
        """报告主智能体最终答案的流式增量片段（前端按片段追加渲染）"""
        self._emit("answer_delta", "正在生成回答", {"delta": delta})
