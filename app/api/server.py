...
# 统一从项目根目录向上查找 .env，保证接口层与工具层读取到同一份配置
...
async def lifespan(_app: FastAPI):
    ...
    # 后台自动同步知识库索引（幂等），不阻塞服务启动；向量模型首次运行需联网下载权重
    asyncio.create_task(_background_rag_sync())
    yield


async def _background_rag_sync() -> None:
    """后台同步全部知识库索引（自研 RAG）"""
    try:
        from app.rag.indexer import sync_all_knowledge_bases

        results = await asyncio.to_thread(sync_all_knowledge_bases, skip_existing=True)
        for result in results:
            print(f"[RAG] 知识库同步完成: {result}")
    except Exception as exc:  # noqa: BLE001
        print(f"[RAG] 知识库自动同步失败: {exc}")
...
# CORS 白名单由 CORS_ORIGINS 控制（逗号分隔多个来源），默认放行本地 Vite 开发地址。
# 注意：不要使用 ["*"] 与 allow_credentials=True 的组合，浏览器会直接拒绝该响应。
_cors_origins = [
    _origin.strip()
    for _origin in os.getenv("CORS_ORIGINS", "http://localhost:5173").split(",")
    if _origin.strip()
]
app.add_middleware(
    CORSMiddleware,
